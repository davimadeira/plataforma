...
export default async function Home() {
  ...
  return <main className="institutional">
    <SiteHeader />
    <section id="inicio" className="person-hero shell"><div className="orbit" /><div className="orbit orbit-two" /><div className="person-copy"><p className="eyebrow"><span /> {content.hero.eyebrow}</p><h1><span>Matheus Vidal</span><br />{content.hero.title}</h1><p>{content.hero.description}</p><div className="actions"><a className="primary agenda-hero-cta" href="#contato">Consultar agenda e convites <span>→</span></a><a className="text-link" href="#sobre">Conheça minha história <span>↓</span></a></div><p className="hero-agenda-note"><b>Agenda ministerial</b> · Convites para cultos, congressos, conferências e eventos.</p></div><div className="person-photo"><div className="orange-halo" /><Image src={content.hero.image} fill sizes="(max-width: 850px) 100vw, 46vw" alt="Matheus Vidal, pregador da Palavra" priority unoptimized /><p>{content.hero.caption}</p></div></section>
    <section className="agenda-priority"><div className="shell agenda-priority-grid"><div><p className="eyebrow gold">AGENDA E CONVITES</p><h2>Leve uma palavra de fé e propósito ao seu evento.</h2><p>Para convites ministeriais, cultos, congressos e conferências, fale diretamente pelo WhatsApp.</p></div><div className="agenda-priority-card"><span>MINISTÉRIO MATHEUS VIDAL</span><strong>Agenda aberta para novos convites.</strong><a href="#contato">Falar sobre a agenda <b>↗</b></a></div></div></section>
