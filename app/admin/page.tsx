...
function UploadProgress({ value }: { value?: number }) {
  ...
  return (
    <small className="admin-image-progress">
      {value < 100
        ? `Enviando imagem: ${value}%`
        : "Imagem enviada — salve para publicar."}
    </small>
  );
}
...
export default function Admin() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([siteSettings, storeProducts, siteContent]) => {
        setSettings({ ...initialSettings, ...siteSettings });
        setProducts(
          Array.isArray(storeProducts) ? storeProducts : defaultProducts,
        );
  ...
  async function saveSite() {
    ...
    setStatus(
      response.ok
        ? "Site institucional publicado com sucesso."
  ...
  async function saveBook() {
    ...
    const failed = !settingsResponse.ok
    ...
    setStatus(
      failed
        ? failed.error || "Não foi possível salvar."
        : "Livro, contato e capa publicados com sucesso.",
    );
  }
  async function saveProducts() {
    ...
    setStatus(
      response.ok
        ...
        : result.error || "Não foi possível salvar os produtos.",
    );
  }
  function updateImage(target: ImageTarget, image: string) {
    setContent((current) =>
      target === "hero"
        ? { ...current, hero: { ...current.hero, image } }
        : target === "book"
          ? { ...current, book: { ...current.book, coverImage: image } }
          : { ...current, store: { ...current.store, image } },
    );
  }
  ...
  async function uploadImage(
    ...
    try {
      const blob = await upload(`site/${target}/${file.name}`, file, {
        access: "public",
        handleUploadUrl: "/api/site-media",
        clientPayload: JSON.stringify({ pin }),
        onUploadProgress: (uploadEvent) =>
          setUploads((current) => ({
            ...current,
            [target]: Math.round(uploadEvent.percentage),
          })),
      });
  ...
  const topTitle = {
    site: "Site e biografia",
    book: "Livro e contato",
    products: "Catálogo da loja",
    school: "Gestão da escola",
  }[tab];
  const saveAction =
    tab === "site"
      ? saveSite
    ...
    <main className="site-admin-page">
      {!allowed ? (
        <section className="admin-login">
          ...
          <span>
            Edite o site, a loja, os livros e a escola em um único lugar.
          </span>
          <input
            type="password"
            value={pin}
          ...
          <aside className="site-admin-sidebar">
            ...
            <nav>
              <button
                ...
                onClick={() => setTab("site")}
              >
                <span>01</span> Site e biografia
              </button>
              <button
                className={tab === "book" ? "active" : ""}
                onClick={() => setTab("book")}
              >
                <span>02</span> Livro e contato
              </button>
              <button
                className={tab === "products" ? "active" : ""}
                onClick={() => setTab("products")}
              >
                <span>03</span> Produtos da loja
              </button>
              <button
                className={tab === "school" ? "active" : ""}
                onClick={() => setTab("school")}
              >
                <span>04</span> Escola e cursos
              </button>
            </nav>
            <div>
              ...
              <button onClick={() => setAllowed(false)}>Sair</button>
            </div>
          </aside>
          <section className="admin-content">
            <header className="admin-topbar">
              <div>
                <p>CENTRAL MV · PAINEL ADMIN</p>
                <h1>{topTitle}</h1>
              </div>
            ...
            {tab === "site" && (
              <section className="admin-general admin-content-editor">
                <div className="admin-section-title">
                  <div>
                    <small>CONTEÚDO DO SITE</small>
                    <h2>Matheus Vidal e sua história</h2>
                  </div>
                  <p>
                    Edite os textos, números e foto que aparecem na página
                    inicial. As alterações só entram no ar depois de salvar.
                  </p>
                </div>
                <div className="admin-form professional">
                  <label>
                  ...
                  <label>
                    Foto principal — endereço da imagem
                    <input
                      value={content.hero.image}
                      onChange={(event) =>
                        updateImage("hero", event.target.value)
                      }
                    />
                  </label>
                  <label>
                  ...
                  <label className="wide admin-image-upload">
                    Enviar nova foto principal
                    <input
                      type="file"
                      accept="image/png,image/jpeg,image/webp"
                      onChange={(event) => void uploadImage("hero", event)}
                    />
                    <UploadProgress value={uploads.hero} />
                  </label>
                  ...
                  <label className="wide">
                    ...
                    <textarea
                      value={content.about.firstParagraph}
                      onChange={(event) =>
                        setContent((current) => ({
                          ...current,
                          about: {
                  ...
                  <label className="wide">
                    ...
                    <textarea
                      value={content.about.secondParagraph}
                      onChange={(event) =>
                        setContent((current) => ({
                          ...current,
                          about: {
                            ...current.about,
                            secondParagraph: event.target.value,
                          },
                        }))
                      }
                    />
                  ...
                  {content.about.stats.map((stat, index) => (
                    <div className="admin-stat-input" key={index}>
                      <label>
                        Número
                        <input
                          value={stat.value}
            ...
            {tab === "book" && (
              <section className="admin-general admin-content-editor">
                <div className="admin-section-title">
                  <div>
                    <small>LIVRO E AGENDA</small>
                    <h2>Obra, capa e contato</h2>
                  </div>
                  <p>
                    Configure o livro, sua capa e o número que recebe os pedidos
                    de agenda no WhatsApp.
                  </p>
                </div>
                <div className="admin-form professional">
                  <label>
                    ...
                    />
                    <small>Recebe os pedidos de agenda e convites.</small>
                  </label>
                  <label>
                    Nome do livro
                    <input
                  ...
                  <label className="wide">
                    ...
                    <input
                      ...
                      placeholder="Opcional — deixe vazio para usar a simulação"
                    />
                  </label>
                  <label>
                    Chamada da seção do livro
                    <input
                  ...
                  <label>
                    Título de apresentação do livro
                    <input
                      value={content.book.title}
                      onChange={(event) =>
                        setContent((current) => ({
                          ...current,
                          book: { ...current.book, title: event.target.value },
                        }))
                      }
                    />
                  </label>
                  ...
                  <label className="wide">
                    Capa do livro — endereço da imagem
                    <input
                      value={content.book.coverImage}
                      onChange={(event) =>
                        updateImage("book", event.target.value)
                      }
                      placeholder="Envie uma imagem abaixo ou cole o endereço"
                    />
                  </label>
                  <label className="wide admin-image-upload">
                    ...
                    <input
                      type="file"
                      accept="image/png,image/jpeg,image/webp"
                      onChange={(event) => void uploadImage("book", event)}
                    />
                    <UploadProgress value={uploads.book} />
                  </label>
                  {content.book.coverImage && (
                    <div className="admin-book-preview wide">
                      <Image
