...
type Section = "courses" | "pix" | "orders" | "security";
...
export default function SchoolEditor() {
  ...
  return <section className="school-editor"><header className="school-editor-head"><div><small>ESCOLA ONLINE · GESTÃO INTEGRADA</small><h2>{section === "courses" ? "Cursos e aulas" : section === "pix" ? "Configurar Pix" : section === "orders" ? "Matrículas e acessos" : "Segurança da escola"}</h2></div><div><a href={schoolUrl} target="_blank" rel="noreferrer">Ver plataforma ↗</a>{(section === "courses" || section === "pix") && <button className="save-button" onClick={() => void save()}>Salvar e publicar</button>}</div></header>
    <nav className="school-editor-nav" aria-label="Seções da escola"><button className={section === "courses" ? "active" : ""} onClick={() => void changeSection("courses")}>Cursos e aulas</button><button className={section === "pix" ? "active" : ""} onClick={() => void changeSection("pix")}>Pagamento Pix</button><button className={section === "orders" ? "active" : ""} onClick={() => void changeSection("orders")}>Matrículas</button><button className={section === "security" ? "active" : ""} onClick={() => void changeSection("security")}>Senha</button></nav>
